 ...
 *   - 敏感字段自动 mask（password / accessToken / refreshToken / secret 等，精确匹配）
 *
...
function toPrismaDeviceType(
  d: RequestUser['deviceType'] | undefined,
): 'CLIENT_APP' | 'RIDER_APP' | 'ADMIN_WEB' | null {
  if (!d) return null;
...
/**
 * 递归 mask 敏感字段（精确匹配，WeakSet 防循环引用）
 *
 * - 精确匹配（lowerK === field）避免 'token' 误杀 'tokenType'
 * - WeakSet 跟踪已访问对象，循环引用返回 '[Circular]'
 */
function maskSensitiveFields(
  ...
  if (Array.isArray(obj)) {
    return obj.map((v) => maskSensitiveFields(v, maskFields, seen));
  }

  const result: Record<string, unknown> = {};
  for (const [k, v] of Object.entries(obj as Record<string, unknown>)) {
    const lowerK = k.toLowerCase();
    if (maskFields.includes(lowerK)) {
      result[k] = '[REDACTED]';
    } else if (v !== null && typeof v === 'object') {
      result[k] = maskSensitiveFields(v, maskFields, seen);
    } else {
      result[k] = v;
    }
  }
...
export class AuditInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    return next.handle().pipe(
      tap({
        next: (responseData) => {
          // 异步写 AuditLog（不阻塞响应）
          this.writeAuditLog(context, method, response.statusCode, responseData, auditOptions).catch(
            (e) => {
              logger.warn({
  ...
  private async writeAuditLog(
    ...
    const resourceId = getResourceId(request.params, options?.resourceIdParam ?? 'id');
    ...
    if (!options?.resource) {
      // 没显式声明 resource 时记 warning（minified 后类名不可靠）
      logger.warn({
        msg: 'audit_resource_not_declared',
        controller: controller?.name,
        url: request.url,
        hint: 'Use @Audit({ resource: "Order" }) for explicit resource name',
      });
    }
